...
**********************************************************************************************************/
-- local 접속 계정
create user 'abc'@'Localhost' identified by '5555';

-- 원격 접속 계정

create user 'abc'@'%' identified by '5555';
...
--  crate table testdb.member
-- 특정 DB를 사용 -> database 이름을 생략하면 설정한 DB를 사용한다.
-- use database이름
use testdb;
-- 사용할 db를 선택한 뒤라면 crate table member 로도 테이블 만들기 가능.
...
insert into member (id, password, name, gender, age) values ('id-4', '3333', '유관순', 'f', '18');

insert into member (id, password, name, gender, age, join_date) 
 values ('id-5', '3333', '유관순', 'f', '18', '2000-10-10');
