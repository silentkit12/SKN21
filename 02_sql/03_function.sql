...
select emp_id,
       emp_name,
       comm_pct,
       round(comm_pct*1.18) as "반올림",
       truncate(comm_pct* 108, 2) as "내림"
from emp 
...
select adddate(now(), interval 3 week) "3주 후",
       adddate(now(), interval 10 hour) "10 시간 후";
       
-- 날짜/시간의 차이 계산
select datediff(curdate(), '2025-10-10'); -- 2925-10-20 -2025 10-10
select timediff(curtime(), '10:10:10');
select timestampdiff(month, '2024-10-10', now());
select timestampdiff(hour, '2024-10-10', now());
...
select emp_name,
       timestampdiff(month, hire_date, curdate()) "근무 개월 수"
from emp
order by 2 desc;
...
select emp_id, emp_name, job, ifnull(dept_name, '----배치 전') "dept_name"
from emp;
...
select salary,
       case when salary >= 10000 then '1등급' else '2등급' end "급여등급"
from emp;
