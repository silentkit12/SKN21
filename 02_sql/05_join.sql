...
select e.emp_id, e.emp_name, j.job_title, d.dept_name
...
from emp e join job j on e.job_id = j.job_id
           ...
           join dept d on e. dept_id = e.dept_id;
...
select d.dept_id, d.dept_name,count(emp_id)"직원수" -- 직원수 > 내가 세려고 하는 테이블의 PK로 카운트
...
select e.emp_id,
	 e.emp_name "직원이름",
     m.emp_name "상사이름",
     date_fromat(e.hire_date, '%Y%m%d') "hire_date"
