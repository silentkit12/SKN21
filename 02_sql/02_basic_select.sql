...
select distinct job, emp_name from emp; -- 두개의 행의 값이 모두 동일하지 않은 것들이 모두 조회


-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일, salary는 급여, dept_name은 소속부서 별칭으로 조회결과를 출력 한다.
select emp_id as 직원ID , -- emp_id 컬럼의 조회 결과를 직원 ID 컬럼으로 보여줘.
...
select emp_id 직원ID , -- as는 공백으로 대체 가능(생략함)
       upper(emp_name) 직원이름,
       hire_date 입사일,
       salary 급여,
       dept_name "소속 부서" -- 띄어쓰기를 할 때에는 ""로 구분
from   emp;
...
select salary, salary*6, salary, salary*salary*120 
From emp;
...
select emp_name, concat('$', salary, '원') from emp;
...
select emp_name, dept_name, dept_name
from emp
...
where comm_pct not between 0.2 and 0.3; -- 0.2~0.3 사이값만 제외
...
from emp
where emp_name not like 'S%'; -- S로 시작하지 않는 것들만
...
where emp_name like '___e%'; -- 글자 수를 지정하고 싶을 때는 언더스코어를 사용.

-- EMP 테이블에서 직원의 이름에 '%' 가 들어가는 직원의 ID(emp_id), 직원이름(emp_name) 조회
--    %나 _ 를 검색하는 값으로 사용할 경우. 
select emp_id, emp_name from emp
where emp_name like '%&%%' escape '&'; -- escape '특수문자'  특수문자 뒤에 %나 _는 조회할 값임을 표시(like의 패턴문자가 아닌)

-- EMP 테이블에서 부서명(dept_name)이 null인 직원의 ID(emp_id), 이름(emp_name), 부서명(dept_name)을 조회.
select emp_id, emp_name, dept_name
from emp
where dept_name is null ; -- 값이 null 인 것을 조회할 경우 = 이 아닌 is null이라는 명령어를 사용.
...
-- where hire_date > '2004-05-31'; -- 2004-05-31 이후 입사한 직원 조회
...
select emp_id, job, dept_name from emp
where job like '%MAN%' 
...
or  dept_name = 'Sales' ; -- (job like '%MAN%' and dept_name = 'Marketing') or  dept_name = 'Sales' 
-- 따라서 ^위의 구문이  in 과 동일한 결과값을 내려면 ()로 묶어야 한다.
