...
select count(ifnull(dept_name, 'a')) from emp;
...
-- comm_pct 평균
select avg(comm_pct),  -- comm_pct가 있는 35명 기준 평균 (null을 제외하고 평균계산)
       avg(ifnull(comm_pct,0)) -- 전체 직원에 대한 평균 (null > 0로 변경한 뒤 평균을 계산)
from emp;
...
select dept_name, job, max(salary), max(salary)
from emp
group by dept_name, job;

-- 급여(salary)가 10,000 이하인 직원수와 10,000초과인 직원수 조회.
select case when salary <= 10000 then '10000 이하'
			  else '10000초과' end "급여 기준",
       count(*) "직원수"
from emp
group by case when salary <= 10000 then '10000 이하'
			  else '10000초과' end;
